...
unsafe fn run() {
	...
	// TODO: Using a custom class is not what we should do here. We should get an instance
	// of the existing IAsyncActionCompletionHandler<T> with the correct T
	// (in the headers there are IIDs defined for every instantiation)
	let myHandler = Box::into_raw(Box::new(MyIAsyncActionCompletedHandler {
		lpVtbl: &MyIAsyncActionCompletedHandlerVtbl,
		count: std::sync::atomic::AtomicUsize::new(1)
	}));
	
	assert!(asyncOp.put_Completed(myHandler as *mut _ as *mut IAsyncActionCompletedHandler) == S_OK);
	println!("HRESULT (put_Completed) = {:x}", hres);
	
	std::thread::sleep(std::time::Duration::from_millis(10)); // wait until async operation has finished
...
const MyIAsyncActionCompletedHandlerVtbl: IAsyncActionCompletedHandlerVtbl = IAsyncActionCompletedHandlerVtbl {
	parent: IUnknownVtbl {
...
unsafe extern "system" fn My_Invoke(this: *mut IAsyncActionCompletedHandler, _asyncAction: *mut IAsyncAction, _status: AsyncStatus) -> HRESULT {
	println!("Hi from My_Invoke ({:p})!", this);
	return S_OK;
}
...
unsafe extern "system" fn My_QueryInterface(this: *mut IUnknown, vTableGuid: REFIID, ppv: *mut *mut VOID) -> HRESULT
{
	...
	print!("Hi from My_QueryInterface: ");
	print_guid(&*vTableGuid);
	let this: &mut MyIAsyncActionCompletedHandler = mem::transmute(this);
	
	// There is no IID for the actual derived (anonymous) class, so we can only check for IID_Unknown
	if *vTableGuid != IID_IUnknown &&
		*vTableGuid != IID_IAsyncActionCompletedHandler &&
		*vTableGuid != IID_IAgileObject &&
		*vTableGuid != KEINE_AHNUNG { // WE SAY THAT IAgileObject IS OKAY -> the object must implement Send! 
	...
	*ppv = this as *mut _ as *mut VOID;

	// Now we call our own AddRef function,
	// passing the IExample.
	((*this.lpVtbl).parent.AddRef)(this as *mut _ as *mut IUnknown);

	// Let him know he indeed has a IExample.
	return S_OK;
}

unsafe extern "system" fn My_AddRef(this: *mut IUnknown) -> ULONG
{
	let this: &mut MyIAsyncActionCompletedHandler = mem::transmute(this);
	// Increment the reference count (count member).
	let old_size = this.count.fetch_add(1, Ordering::Relaxed);
...
unsafe extern "system" fn My_Release(this: *mut IUnknown) -> ULONG
{
	
	let this: &mut MyIAsyncActionCompletedHandler = mem::transmute(this);
	
	let old_size = this.count.fetch_sub(1, Ordering::Release);
	...
	drop(this); // Arc uses a trick to call this in an inline(never) function
	return 0;
}
