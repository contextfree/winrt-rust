...
fn main() {
    ...
    let to_string = uri.query_interface::<IStringable>().unwrap().deref().to_string().unwrap();
    println!("{} -> {}", uri.get_runtime_class_name(), to_string);
    println!("TrustLevel: {:?}", uri.deref().get_trust_level());
    println!("GetIids:");
    let iids = uri.deref().get_iids();
    for i in 0..iids.len() {
        println!("  [{}] = {:?}", i, iids[i]);
    }
    ...
    let id = asi.deref().get_id().unwrap();
    println!("id: {:?}", id);
    let status = asi.deref().get_status().unwrap();
    println!("status: {:?}", status);

    let device_information_collection = async_op.deref().blocking_get().unwrap().unwrap();
    println!("CLS: {}", device_information_collection.get_runtime_class_name());
    let count = device_information_collection.deref().get_size().unwrap();
    println!("Device Count: {}", count);
    
    let mut remember = None;
    let mut i = 0;
    for current in device_information_collection.deref().into_iter() {
        let current = current.expect("current was null");
        let device_name = current.deref().get_name().unwrap();
    ...
    device_information_collection.deref().get_many(0, &mut buffer).unwrap();
    for (b, i) in buffer.iter().zip(0..) {
        let device_name = b.deref().get_name().unwrap();
        println!("Device Name ({}): {}", i, device_name);
    }
    let len = buffer.len();
    drop(buffer);
    ...
    if let Some(mut r) = remember {
        let (index, found) = device_information_collection.deref().index_of(&mut r).unwrap();
        println!("Found remembered value: {} (index: {})", found, index);
        assert_eq!(index, 100);
    }
    
    match device_information_collection.deref().get_at(count + 42) {
    ...
    assert_eq!(boxed_array.deref().get_type().unwrap(), PropertyType::BooleanArray);
    let boxed_array = boxed_array.query_interface::<IReferenceArray<bool>>().unwrap();
    let returned_array = boxed_array.deref().get_value().unwrap();
    ...
    let boxed_array = boxed_array.query_interface::<IPropertyValue>().unwrap();
    assert_eq!(boxed_array.deref().get_type().unwrap(), PropertyType::StringArray);
    ...
    let file = StorageFile::get_file_from_path_async(&*FastHString::new(&exe_path_str)).unwrap().deref().blocking_get().expect("get_file_from_path_async failed").unwrap();
    println!("Executable file: {}", file.query_interface::<IStorageItem>().unwrap().deref().get_path().unwrap());
    ...
    let exe_folder = file.query_interface::<IStorageItem2>().unwrap().deref().get_parent_async().unwrap().deref().blocking_get().expect("get_parent_async failed").unwrap();    
    let txt_file = exe_folder.deref().create_file_async(&*FastHString::new("__test_file.txt"), CreationCollisionOption::ReplaceExisting).unwrap().deref().blocking_get().expect("create_file_async failed").unwrap();
    println!("Created text file {}", txt_file.query_interface::<IStorageItem>().unwrap().deref().get_path().unwrap());
    FileIO::append_text_async(&txt_file, &*FastHString::new("This is a test\nand a second line.")).unwrap().deref().blocking_wait();
    let mut lines: ComPtr<IVector<HString>> = FileIO::read_lines_async(&txt_file).unwrap().deref().blocking_get().expect("read_lines_async failed").unwrap();
    println!("Read {} lines from the text file", lines.deref().get_size().expect("get_size failed"));
    // now we have an IVector that we can mess with
    lines.deref_mut().append(&*FastHString::new("The third line, added later")).expect("append failed");
    assert_eq!(lines.deref().get_size().unwrap(), 3);
    println!("=== Lines: ===");
    for line in &lines {
        println!("{}", line);
